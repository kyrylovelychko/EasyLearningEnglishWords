...
public class DatabaseContentProvider extends ContentProvider {
    ...
    private static final int ONE_DICTIONARY = 101; // Оин словарь
    private static final int DICTIONARIES = 102; // Таблица словарей
    ...
    static {
        ...
        uriMatcher.addURI(DatabaseDescription.AUTHORITY,
                DatabaseDescription.Dictionaries.TABLE_NAME + "/#", ONE_DICTIONARY);

        // Uri для всех записей таблицы словарей
        uriMatcher.addURI(DatabaseDescription.AUTHORITY,
                DatabaseDescription.Dictionaries.TABLE_NAME, DICTIONARIES);

        // Uri для слова с заданным идентификатором
        uriMatcher.addURI(DatabaseDescription.AUTHORITY,
                DatabaseDescription.Words.TABLE_NAME + "/#", ONE_WORD);

        // Uri для всех записей таблицы слов
        uriMatcher.addURI(DatabaseDescription.AUTHORITY,
                DatabaseDescription.Words.TABLE_NAME, WORDS);
    }
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        switch (uriMatcher.match(uri)) {
            case ONE_DICTIONARY:
                ...
                queryBuilder.appendWhere(DatabaseDescription.Dictionaries._ID + "="
            ...
            case ONE_WORD:
                ...
                queryBuilder.appendWhere(DatabaseDescription.Words._ID + "="
                        + uri.getLastPathSegment());
                break;
            case WORDS:
                queryBuilder.setTables(DatabaseDescription.Words.TABLE_NAME);
        ...
        switch (uriMatcher.match(uri)) {
            case WORDS:
                rowId = dbHelper.getWritableDatabase()
                        .insert(DatabaseDescription.Words.TABLE_NAME, null, values);
                if (rowId > 0) {
            ...
            case DICTIONARIES:
                rowId = dbHelper.getWritableDatabase()
                        .insert(DatabaseDescription.Dictionaries.TABLE_NAME, null, values);
                if (rowId > 0) {
        ...
        switch (uriMatcher.match(uri)) {
            case ONE_WORD:
                ...
                numberOfRowsUpdated = dbHelper.getWritableDatabase()
                        .update(DatabaseDescription.Words.TABLE_NAME,
            ...
            case ONE_DICTIONARY:
                id = uri.getLastPathSegment();
                numberOfRowsUpdated = dbHelper.getWritableDatabase()
                        .update(DatabaseDescription.Dictionaries.TABLE_NAME,
                                values, DatabaseDescription.Dictionaries._ID + "="
                                        + id, selectionArgs);
                break;
            default:
